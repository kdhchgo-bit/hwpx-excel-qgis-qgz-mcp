---
name: hwpx-local
description: Inspect, validate, search, extract, copy-edit, native-open, or export local Hangul HWPX files on Windows, including table morphology analysis for merged cells, headers, body rows, bottom form rows, fills, and inner/outer borders. Use when the user names an .hwpx file, asks to preserve Hangul layout, edit or understand a table, replace HWPX text, check package integrity, extract document text, or convert HWPX to PDF with installed Hancom Hangul.
---
...
4. For an edit, use an explicit new `output_path`. Keep `overwrite=false` unless the user explicitly authorizes replacing that exact output.
5. Run `hwpx_validate`, `hwpx_inspect`, or `hwpx_find_text` on the output and report its full path.
6. Use native Hangul operations only for open verification or PDF export; they run hidden and close after use.
